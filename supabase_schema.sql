...
CREATE TABLE IF NOT EXISTS public.profiles (
  ...
  role TEXT NOT NULL DEFAULT 'Membro da Comunidade',
  family_tag TEXT,
  tag TEXT,
...
BEGIN
  ...
  ELSIF TG_OP = 'UPDATE' THEN
    IF NEW.role IS DISTINCT FROM OLD.role AND NOT public.is_admin() THEN
      NEW.role := OLD.role;
    END IF;
    -- 🛡️ BLINDAGEM: Impede que membros banidos desbanam a si mesmos
    IF (
...
-- ========================================================
-- OBSERVAÇÃO DE SEGURANÇA [SEC-EDGE-01]:
-- A troca de e-mail é realizada exclusivamente pela Edge Function 'update-user-email',
-- que exige re-autenticação com senha atual e validação de token OTP para prevenir Account Takeover.
-- A função RPC update_own_email foi descontinuada e removida.
-- ========================================================
...
-- Leitura pública dos stories da Home
CREATE POLICY "destaques_select_public" ON public.destaques
  FOR SELECT USING (true);

-- 🛡️ BLINDAGEM: Somente administradores podem criar, editar ou excluir stories/destaques
...
-- --------------------------------------------------------
-- 🛡️ BLINDAGEM LGPD: Permite leitura de check-ins emocionais estritamente pelo próprio autor ou administradores
DROP POLICY IF EXISTS "checkins_select_own" ON public.emotional_checkins;
...
-- ========================================================
-- 22. ÍNDICES DE PERFORMANCE E ESCALABILIDADE (PRÉ-LANÇAMENTO)
-- ========================================================
...
-- ========================================================
-- 23. DIREITO AO ESQUECIMENTO / EXCLUSÃO DE CONTA (LGPD ART. 18 / APPLE GUIDELINE 5.1.1)
-- ========================================================
